...
**Marketing site**: [chute.pages.dev](https://chute.pages.dev)
...
Chute is a single keyboard shortcut that opens a tiny capture box on top of whatever you're doing. Type the thought, hit Enter, and Chute routes it to the right app based on what you wrote. Calendar gets the time-stamped ones, Apple Notes gets the rest, and you never had to switch context.
...
|---|---|---|
| `chute.db` (SQLite) | Captures, routing decisions, sync log | Structured queries, atomic transactions, scales to 100k+ records |
...
Storage path per OS:
- **macOS**: `~/Library/Application Support/com.usechute.app/`
- **Windows**: `%APPDATA%\com.usechute.app\`

The `com.usechute.app` directory name comes from the bundle identifier set in `src-tauri/tauri.conf.json`.
...
```mermaid
sequenceDiagram
  participant App as Chute desktop
  participant Bridge as chute-auth (cloud)
...
After this completes, the bridge gets out of the way. Chute talks to Discord directly for every subsequent capture.

The only ongoing bridge involvement: Google access tokens expire every hour, so Chute calls `POST /api/google/refresh-token` with the refresh token, and the bridge uses `GOOGLE_CLIENT_SECRET` to mint a new access token.
...
```mermaid
flowchart TD
  subgraph User ["~/Library/Application Support/com.usechute.app/"]
    DB["chute.db (SQLite)"]
    Secure["secure-settings.json"]
    Store["preferences JSON"]
  end
  subgraph Tables ["chute.db tables"]
...
|---|---|---|
| `chute.db` | Captures, routing decisions | Standard file permissions |
...
Tauri's built-in event bus. The Rust core or any frontend window can `emit` events that any other window can `listen` for, namespaced with `chute://`. Saving a capture in the overlay emits `chute://capture-saved`, which the Library window picks up and re-renders. Avoids polling or shared state machines.

### How did you handle Apple Notes given there's no official API?

AppleScript via `osascript`, shelled from Rust. The script is hardcoded; user-supplied content is passed as `argv` instead of being interpolated into the script string, so script injection is impossible. The Notes folder is auto-created if missing. Falls back to a clear "grant Chute Automation permission in System Settings" error on failure.
...
- iCloud sync of `chute.db` via the user's existing iCloud Drive (opt-in)
